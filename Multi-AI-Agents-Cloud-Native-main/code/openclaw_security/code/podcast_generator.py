#!/usr/bin/env python3
"""
podcast_generator.py  —  Bibliothèque de génération de dialogues de podcast.
Appelé par auto_run.py ; reçoit un objet TrendTopic et écrit un fichier TXT.
Pas d'interface CLI interactive — tous les paramètres sont fournis par auto_run.py.
"""
...
#  Configuration
# ─────────────────────────────────────────────
# Variables globales pour les connexions Ollama / OpenClaw et paramètres d'application
...
def _load_token() -> str:
    """
    Charge le token du gateway depuis un fichier ou une variable d'environnement.
    Priorise OPENCLAW_TOKEN_FILE (roté par secrets-init à chaque démarrage).
    Correspond au pattern secrets virtiofs dans ryoooo/microvm-openclaw.nix.
    """
...
#  Appel LLM
# ─────────────────────────────────────────────
# Envoi des messages au backend Ollama et récupération de la réponse texte.
...
# ─────────────────────────────────────────────
#  Analyse du dialogue et vérification de qualité
# ─────────────────────────────────────────────
# Convertit le texte brut en tours de parole (host/guest) et compte les tours valides.
...
#  Sortie formatée
# ─────────────────────────────────────────────
# Génère le contenu final du fichier podcast .txt avec en-tête, dialogue, résumé et méta.
...
# ─────────────────────────────────────────────
# Vérifie l'accessibilité d'Ollama et d'OpenClaw avant démarrage.
...
# ─────────────────────────────────────────────
#  Fonction principale de génération (appelée par auto_run.py)
# ─────────────────────────────────────────────
# Exécute DeepSearch, sollicite l'LLM, vérifie la qualité et sauvegarde le résultat.
