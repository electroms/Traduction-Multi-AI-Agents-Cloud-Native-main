"""
trend_scout.py  —  Module de recherche de tendances.
Appelle l'endpoint OpenClaw Gateway /v1/chat/completions,
demande à l'agent trend-scout d'utiliser des outils web_search pour trouver
les sujets IA/tech les plus populaires du moment, puis renvoie une liste de Topic structurées.
"""
...
def _load_token() -> str:
    """
    Charge le token de la passerelle depuis un fichier ou une variable d'environnement.
    Priorise OPENCLAW_TOKEN_FILE (roté par secrets-init à chaque démarrage).
    """
...
# ─────────────────────────────────────────────
#  Appel de l'agent OpenClaw (avec retries + parsing de flux)
# ─────────────────────────────────────────────
...
def _call_openclaw_agent(
    ...
    """
    Appelle l'endpoint OpenClaw Gateway /v1/chat/completions.
    Utilise le modèle "openclaw:<agentId>" pour cibler un agent spécifique.
    L'agent exécute des outils web_search puis renvoie le texte final.

    Réf : https://fossies.org/linux/openclaw/docs/gateway/openai-http-api.md
    """
...
# ─────────────────────────────────────────────
#  Parsing JSON avec tolérance d'erreurs
# ─────────────────────────────────────────────

def _parse_topics(raw: str) -> list[TrendTopic]:
    """Extrait la liste de Topic en JSON de la réponse de l'agent (tolère le texte additionnel et les blocs <think>)"""
    # Supprime les blocs <think>...</think> émis par les modèles de raisonnement (ex : qwen3)
...
def get_trending_topics(top_n: int = 3) -> list[TrendTopic]:
    """
    Appelle l'agent trend-scout OpenClaw et retourne les sujets AI/tech actuels.

    Flux :
    1. POST /v1/chat/completions → OpenClaw Gateway
    2. agent trend-scout exécute des requêtes web_search
    3. l'agent synthétise les résultats et renvoie du JSON structuré
    4. Parse et retourne la liste TrendTopic

    Args:
        top_n: Nombre de sujets à retourner (par défaut 3)
    Returns:
        Liste TrendTopic triée par score de tendance décroissant
    """
    print(f"\n{'='*50}")
...
def get_trending_topics_fallback(top_n: int = 3) -> list[TrendTopic]:
    """
    Solution de secours : quand OpenClaw est indisponible, recherche les tendances directement via SerpAPI.
    Contourne l'agent OpenClaw en mode dégradé.
    """
...
def scout_with_fallback(top_n: int = 3) -> list[TrendTopic]:
    """
    Point d'entrée principal : utilise SerpAPI directement pour la découverte de tendances.
    Le chemin avec OpenClaw agent est disponible via get_trending_topics() si nécessaire.
    """
